...
def create_app_mappings(metadata: Dict[str, Any]) -> tuple[
    ...
    """
    restricted_field_names = ['id']  # if teh Knack User defined a field with the name 'id', it will be overwritten
    ...
    for obj in metadata['application']['objects']:
        ...
        for field in obj['fields']:
            ...
            if field_name.lower().strip() in restricted_field_names:
                field_name = f"{inflections.get('singular')}_{field_name.lower()}"
            ...
            if field['type'] in ['boolean']:
                if 'default' in field['format']:
                    field_default_value = field['format']['default']
